...
/**
 * Modified by jim on 10/11/15.
 */
public class ModifiedImprovedBacktrackingStrategy extends ImprovedBacktrackingStrategy
{
	...
	public ModifiedImprovedBacktrackingStrategy()
	{
	}

	/** Creates a backtracking strategy with the specified features. */
	public ModifiedImprovedBacktrackingStrategy(boolean enableMRV, boolean enableDeg, boolean enableAC3, boolean enableLCV)
	{
		if (enableMRV)
	...
	/**
	 * Primitive operation, which tries to prune out values from the CSP which are not possible anymore when extending
	 * the given assignment to a solution.
	 *
	 * @return An object which provides informations about (1) whether changes have been performed, (2) possibly
	 * inferred empty domains , and (3) how to restore the domains.
	 */
	@Override
	protected DomainRestoreInfo inference(Variable var, Assignment assignment, CSP csp)
	{
		switch (inferenceStrategy)
		{
			...
			case MAC:
				DomainRestoreInfo fcInfo = doForwardChecking(var, assignment, csp);
				return new ModifiedAC3Strategy().reduceDomains(csp,fcInfo);
			default:

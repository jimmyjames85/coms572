...
import aima.gui.framework.*;
		import project2.UnitSquareMap;

		import java.awt.*;

/**
 * Application which demonstrates basic constraint algorithms based on map coloring problems. It shows the constraint
 * graph, lets the user select a solution strategy, and allows then to follow the progress step by step. For pragmatic
 * reasons, the implementation uses the agent framework, even though there is no agent and only a dummy environment.
 *
 * @author Ruediger Lunde
 */

/**
 * Modified by jim on 10/12/15.
 */
public class ModifiedMapColoringApp extends SimpleAgentApp
{
	...
	{
		new ModifiedMapColoringApp().startApplication();
	}
	...
	protected static class MapColoringFrame extends AgentAppFrame
	{
		...
		public MapColoringFrame()
		{
			...
			setSelectors(new String[]{ENV_SEL, STRATEGY_SEL}, new String[]{"Select Environment", "Select Solution Strategy"});
			setSelectorItems(ENV_SEL, new String[]{"Map of Australia", "Map of Australia NSW=BLUE (for LCV)", "Map of Australia WA=RED (for LCV)"}, 0);
			setSelectorItems(STRATEGY_SEL, new String[]{"Backtracking", "Backtracking + MRV & DEG", "Backtracking + Forward Checking", "Backtracking + Forward Checking + MRV", "Backtracking + Forward Checking + LCV", "Backtracking + AC3", "Backtracking + AC3 + MRV & DEG + LCV", "Min-Conflicts (50)"}, 0);
			setEnvView(new ModifiedCSPView());
			setSize(800, 600);
		}
	}
	...
	protected static class MapColoringController extends AgentAppController
	{
		...
		public void prepare(String changedSelector)
		{
			...
			*/
			...
			Rectangle mapDimensions = new Rectangle(0,0,40,40);
			UnitSquareMap usm = new UnitSquareMap(12, true, mapDimensions);
			usm.updateCSPView(view);
			csp = usm;
			actions.clear();
			actionCount = 0;
			env.init(csp);
		...
		{
			return env.getCSP() != null && (!actions.isEmpty() || env.getAssignment() == null);
		}
		...
		/**
		 * Starts the selected constraint solver and fills the action list if necessary.
		 */
